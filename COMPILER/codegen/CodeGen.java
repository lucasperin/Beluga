...
public class CodeGen extends TypeCheck {
    ...
    public void CodeGenClassDeclNode(ClassDeclNode x) throws GenCodeException {
        ...
        putCode(";------------------------------------------------");
        putCode("; Code generated by Beluga compiler");
        putCode("; Version 1.0 - 2013");
        putCode(";------------------------------------------------");
    ...
    public void CodeGenClassBodyNode(ClassBodyNode x) throws GenCodeException {
        ...
        l = Curtable.methodFindInclass("construtor", null);
    ...
    public void CodeGenMethodDeclNode(MethodDeclNode x) {
        ...
        if (t.name.equals("iniciar") && (r == null)) // m�todo start 
    ...
    public void CodeGenPrintNode(PrintNode x) {
        ...
        //XXX Codigo novo, adicao de imprimir Inteiro 
        if (t.ty == STRING_TYPE ) 
            // chama PrintStream.print(String)
            putCode("invokevirtual java/io/PrintStream/print(Ljava/lang/String;)V",
                -2);
        else if(t.ty == INT_TYPE)
        {
            // chama PrintStream.print(int)
            putCode("invokevirtual java/io/PrintStream/print(I)V",
                -2);
        }
    }
    ...
    public void CodeGenReadNode(ReadNode x) {
        ...
        if (t.ty == INT_TYPE) { // chama readInt, resultado na pilha
            putCode("invokestatic langBrt/Runtime/readInt()I", 1);
        } else { // l� string chama readString, resultado na pilha
            putCode("invokestatic langBrt/Runtime/readString()Ljava/lang/String;",
                1);
        }
    ...
    public type CodeGenRelationalNode(RelationalNode x) {
        ...
        //XXX inserido Boolean nas comparacoes de Equal, NotEqual, AND, OR
        } 
        else if((t1.dim == 0) && (t1.ty == BOOLEAN_TYPE))
        {
        	 if (op == langBConstants.EQ)
        		 putCode("if_icmpeq relexpr" + lab, -2);
        	 else if(op == langBConstants.NEQ)
        		 putCode("if_icmpne relexpr" + lab, -2);
        	 else //
        	 {
            	 if (op == langBConstants.OR)
            		 putCode("ior", -2);
            	 else if(op == langBConstants.AND)
            		 putCode("iand", -2);
            ...
            if (op == langBConstants.EQ) { // s� == ou != s�o permitidos
    ...
    public type CodeGenVarNode(VarNode x) {
        ...
        if (p.localcount >= 0) { // é variável local
        	//XXX Adicionado Boolean
            if ((p.type == INT_TYPE || p.type == BOOLEAN_TYPE) && (p.dim == 0)) { // escolhe iload ou aload
    ...
    public type CodeGenExpreNode(ExpreNode x) {
        if (x instanceof NewObjectNode) {
            ...
            return CodeGenVarNode((VarNode) x);
    			...
    			return CodeGenBooleanConstNode((BooleanConstNode) x);
    		}
    		else
        {
            return null;
    ...
    public void CodeGenStatementNode(StatementNode x) {
        if (x instanceof BlockNode) {
			...
			CodeGenWhileStatNode((WhileStatNode) x);
		} else if(x instanceof SwitchStatNode) {
			CodeGenSwitchStatNode((SwitchStatNode) x);
		}
    }

    private String selectBinInstruct(int op) {
        switch (op) {
        case langBConstants.PLUS:
            return "iadd";

        case langBConstants.MINUS:
            return "isub";

        case langBConstants.STAR:
            return "imul";

        case langBConstants.SLASH:
            return "idiv";

        case langBConstants.REM:
            return "irem";

        case langBConstants.EQ:
            return "if_icmpeq";

        case langBConstants.NEQ:
            return "if_icmpne";

        case langBConstants.LT:
            return "if_icmplt";

        case langBConstants.GT:
            return "if_icmpgt";

        case langBConstants.LE:
            return "if_icmple";

        case langBConstants.GE:
            return "if_icmpge";
            
        case langBConstants.OR:
            return "ior";

        case langBConstants.AND:
            return "iand";
        }
    ...
    private void createMain() {
        ...
        putCode(".limit  locals 1");
        putCode(".limit  stack 1");
        putCode("invokestatic langBrt/Runtime/initialize()I");
        putCode("ifne         end");
        putCode("invokestatic " + v.completeName() + "/iniciar()I");
        putCode("pop");
        putLabel("end:");
        putCode("invokestatic langBrt/Runtime/finilizy()V");
        putCode("return");
        putCode(".end method");
    }
    ...
    //XXX
	// -------------------------- Constante boleana ----------------------
    public type CodeGenBooleanConstNode(BooleanConstNode x) {
        if (x == null) {
            return null;
        }

        // carrega a constante na pilha
		int bool = x.position.image.equals("cara") ? 1 : 0;
	...
	public void CodeGenWhileStatNode(WhileStatNode x) {
        if (x == null) {
            return;
        }
        ...
        putCode();
        putCode(";begins whileloop " + lab);
        
        // gera label para laço do comando
        putLabel("whileloop" + lab + ":");
	...
	public void CodeGenSwitchStatNode(SwitchStatNode x){
		if(x == null){
			return;
		}
		...
		boolean b = store;
		store = false;
		ArrayList<String> cLabs = new ArrayList();
		ArrayList<CaseStatNode> caseNodeList = new ArrayList();
		String jat = "jat" + newLabel();

		String oldbreaklabel = breaklabel;			
		
		CaseListNode c = x.caselist;
		while(c != null){
			caseNodeList.add(c.casenode);	
			c = c.caselist;				
		}	

		int size = caseNodeList.size();

		if(size > 0){
			breaklabel = "rof" + newLabel();

			putCode();

			EntryVar p = Curtable.varFind(x.expr.position.image);
			String cmpInst = p.type == INT_TYPE ? "if_icmpne" : "if_acmpne";
		
			for(int i = 0; i < size; i++){
				String next = null;

				CaseStatNode c1 = caseNodeList.get(i);
				cLabs.add("case" + newLabel());

				CodeGenExpreNode(x.expr);
				CodeGenExpreNode(c1.expr);
			

				if(i + 1 < size){
					next = "next" + newLabel();
					putCode(cmpInst + " " + next, -2);					
				}else{							
					putCode(cmpInst + " " + jat, -2);
				}

				putCode("pop", -1); //desempilja inteiro para o default
				putCode("ldc " + i);	
			...
			putLabel(jat + ":");
			putLabel("tableswitch 0");

			for(String l : cLabs){
				putCode(l);
			}

			String dLab = "default" + newLabel();
			putCode("default : " + dLab);

			for(int i = 0; i < cLabs.size(); i++){
				String lab = cLabs.get(i);
				putLabel(lab + ":");
				CodeGenStatementNode(caseNodeList.get(i).stat);
			}
